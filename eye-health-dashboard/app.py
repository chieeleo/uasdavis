...
Native Streamlit version of the Plotly.js dashboard (index.html). It reads the
same auto-generated data.js that the HTML uses, so there is a single source of
truth and no local Excel file is needed — it runs unchanged on Streamlit Cloud.
...
st.markdown(
    """
    <style>
      /* ---- pin a LIGHT look no matter the viewer's system / Cloud theme ---- */
      :root { color-scheme: light; }
      .stApp {
          background:
            radial-gradient(1200px 500px at 80% -8%, #d9efea 0%, transparent 55%), #eef4f3 !important;
      }
      [data-testid="stHeader"] { background: rgba(0,0,0,0) !important; }
      .block-container { padding-top: 1.6rem; padding-bottom: 3rem; max-width: 1320px; }

      /* ---- text: force dark ink everywhere (default theme may render light) ---- */
      .stApp, .stApp p, .stApp li, .stApp label,
      h1, h2, h3, h4, h5, h6,
      [data-testid="stMarkdownContainer"], [data-testid="stMarkdownContainer"] *,
      [data-testid="stWidgetLabel"], [data-testid="stWidgetLabel"] * {
          color: #0f2a30 !important;
      }
      h1, h2, h3 { letter-spacing: -.4px; }
      /* captions / small helper text a softer slate */
      [data-testid="stCaptionContainer"], [data-testid="stCaptionContainer"] *,
      .stApp small { color: #52696f !important; }

      /* ---- sidebar ---- */
      section[data-testid="stSidebar"] { background:#ffffff !important; border-right:1px solid #dde8e6; }
      section[data-testid="stSidebar"] * { color:#0f2a30 !important; }
      ...
      [data-testid="stMetricLabel"], [data-testid="stMetricLabel"] * {
          color:#7e9298 !important; font-weight:600;
          text-transform:uppercase; letter-spacing:.6px; font-size:11.5px; }
      [data-testid="stMetricValue"], [data-testid="stMetricValue"] * { color:#0f2a30 !important; }
      [data-testid="stMetricDelta"], [data-testid="stMetricDelta"] * { color:#0d9488 !important; }
      [data-testid="stMetricDelta"] svg { display:none; }   /* drop the misleading up-arrow */

      /* ---- teal accents in place of Streamlit's default red primary ---- */
      ...
      [data-testid="stSliderThumbValue"] { color:#0d9488 !important; }
      [data-baseweb="checkbox"] [aria-checked="true"],
      [data-baseweb="checkbox"] [data-checked="true"] {
          background-color:#0d9488 !important; border-color:#0d9488 !important; }

      .eh-note { color:#52696f !important; font-size:13px; background:#ffffff; border:1px solid #dde8e6;
          border-radius:14px; padding:12px 16px; box-shadow:0 8px 24px rgba(15,42,48,.05); }
      .eh-note b { color:#0f2a30 !important; }
      .eh-foot { color:#7e9298 !important; font-size:12px; text-align:center; margin-top:18px; line-height:1.6; }
    </style>
    """,
...
def load_data() -> pd.DataFrame:
    """Parse the same data.js the HTML dashboard uses (window.EYE_DATA = {...};)."""
    src = (Path(__file__).parent / "data.js").read_text(encoding="utf-8")
    m = re.search(r"window\.EYE_DATA\s*=\s*(\{.*\})\s*;", src, re.S)
    payload = json.loads(m.group(1))
    cols = payload["columns"]
    return pd.DataFrame({c: payload[c] for c in cols})[cols]
